...
class PayrollReport(models.AbstractModel):
    _name = 'report.payroll_print_xlsx.xlsx_payroll_report' 
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        for used_struct in used_structures:
            ...
            sheet.write(2,0,'Number',format1)
            sheet.write(2,1,'Name',format1)
            sheet.write(2,2,'Department',format1)
            sheet.write(2,3,'Location',format1)
            sheet.write(2,4,'Start Date',format1)
            sheet.write(2,5,'ID/Igama',format1)
            sheet.write(2,6,'Bank ID',format1)
            sheet.write(2,7,'IBAN',format1)
            ...
            for slip in lines.slip_ids:
                if lines.slip_ids:
                    if slip.struct_id.id == used_struct[0]:
                        has_payslips = True
                        sheet.write(e_name, 0, slip.employee_id.employee_no, format3)
                        sheet.write(e_name, 1, slip.employee_id.name, format3)
                        sheet.write(e_name, 2, slip.employee_id.department_id.name, format3)
                        sheet.write(e_name, 3, slip.employee_id.address_home_id.name, format3)
                        sheet.write(e_name, 4, str(slip.contract_id.date_start.strftime('%B %d, %Y')), format3)
                        sheet.write(e_name, 5, slip.employee_id.identification_id, format3)
                        sheet.write(e_name, 6, slip.employee_id.bank_account_id.bank_id.name, format3)
                        sheet.write(e_name, 7, slip.employee_id.bank_account_id.acc_number, format3)
