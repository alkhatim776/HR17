...
class PayslipReport(models.AbstractModel):
    _name = 'report.hr_payroll_report_xlsx.hr_payroll_report_excel' 
            ...
            for slip in lines.slip_ids:
                if lines.slip_ids:
                    if slip.struct_id.id == used_struct[0]:
                        ...
                        sheet.write(e_name, 4, slip.contract_id.date_start, format3)
                        sheet.write(e_name, 5, slip.employee_id.identification_id, format3)
                        sheet.write(e_name, 6, slip.employee_id.bank_account_id, format3)
                        sheet.write(e_name, 6, slip.employee_id.bank_account_id, format3)
