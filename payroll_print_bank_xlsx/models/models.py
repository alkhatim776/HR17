...
class PayrollReportBank(models.AbstractModel):
    _name = 'report.payroll_print_bank_xlsx.xlsx_payroll_bank_report' 
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        for used_struct in used_structures:
            ...
            sheet.write(2,0,'ID/Igama',format1)
            sheet.write(2,1,'Name',format1)
            sheet.write(2,2,'Bank ID',format1)
            sheet.write(2,3,'IBAN',format1)
            

            # for rule in rules:
            #     sheet.write(2,rule[0],rule[2],format1)
            sheet.write(2,4,'Gross Salary',format1)
            sheet.write(2,5,'Basic Salary',format1)
            sheet.write(2,6,'House Rent Allowance',format1)
            sheet.write(2,7,'Addition',format1)
            sheet.write(2,8,'Deduct',format1)

            # sheet.write(2,9,'Employee No',format1)
            sheet.write(2,9,'Status',format1)
            sheet.write(2,10,'Employee No',format1)
            # sheet.write(2,12,'Company',format1)
            sheet.write(2,11,'Location',format1)
            ...
            for slip in lines.slip_ids:
                ...
                if lines.slip_ids:
                    if slip.struct_id.id == used_struct[0]:
                        has_payslips = True
                        if slip.employee_id.is_stranger == False:
                            sheet.write(e_name, 0, slip.employee_id.identification_id, format3)
                        else :
                            sheet.write(e_name, 0, slip.employee_id.iqama_id.name, format3)
                        sheet.write(e_name, 1, slip.employee_id.name, format3)
                        if slip.employee_id.bank_account_id.bank_id.name:
                            sheet.write(e_name, 2, slip.employee_id.bank_account_id.bank_id.name, format3)
                        else:
                            sheet.write(e_name, 2, '-', format3)
                        if slip.employee_id.bank_account_id.acc_number:
                            sheet.write(e_name, 3, slip.employee_id.bank_account_id.acc_number, format3)
                        else:
                             sheet.write(e_name, 3, '-', format3)
                        # sheet.write(e_name, 9, slip.employee_id.id, format3)
                        active='UnActive'
                        if slip.employee_id.active:
                        	active='Active'
                        sheet.write(e_name, 9, active, format3)
                        sheet.write(e_name, 10, slip.employee_id.employee_no, format3)
                        # sheet.write(e_name, 12, slip.employee_id.address_id.name, format3)
                        sheet.write(e_name, 11, slip.employee_id.address_home_id.name, format3)
                        
                        for line in slip.line_ids:
                            if line.category_id.code == 'BASIC':
                                basic+=line.amount
                            if line.category_id.code == 'HRA':
                                hra+=line.amount
                            if line.category_id.code == 'ALW':
                                alw+=line.amount
                            if line.category_id.code == 'DED':
                                ded+=line.amount
                        gross=(basic+hra+alw)-(ded)   
                            # for rule in rules:
                            
                        sheet.write(x, 4, gross, format3_colored)
                        sheet.write(x, 5, basic, format3_colored)
                        sheet.write(x, 6, hra, format3_colored)
                        sheet.write(x, 7, alw, format3_colored)
                        sheet.write(x, 8, ded, format3_colored)
                        # for line in slip.line_ids:
                        #     for rule in rules:
                                # if line.code == rule[1]:
                                #     if line.amount > 0:
