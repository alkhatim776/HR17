...
class HrDeputations(models.Model):
    ...
    housing_by = fields.Selection(
        [('company', 'By Company'), ('employee', 'By Employee')], string='Hotel Reservation',required=True,)
    ...
    is_ticket_created = fields.Boolean(string="Is Ticket Created")
    ...
    def count_tickets(self):
        for rec in self:
            rec.ticket_count = self.env['hr.ticket.request'].search_count([('deputation_id', '=', rec.id)])
    ...
    def action_hr_salary_accountant_approve(self):
        ...
        if self.employee_id.country_code != 'SA' and self.deputation_type == 'external':
            vals = {
                'request_for': 'employee',
                'travel_before_date': fields.date.today(),
                'exit_return_type': 'multi',
        ...
        if not self.book_ticket_check:
            if self.state == 'hr_salary_accountant' and self.travel_by == 'air':
                if self.ticket_cost > 0:
                    hr_ticket_request_type = self.env['hr.ticket.request.type'].search([], limit=1)
                    vals = {
        ...
        move_id = self.env['account.move'].sudo().create([
            {
                ...
                'line_ids': [
                    (0, 0, {
                        'name': _('Deputation Cost For: {} - Period {} - {}').format(self.employee_id.name,
